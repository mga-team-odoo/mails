...
#    Copytright (C) 2017 Chrisotphe CHAUVET, Mirounga
#
...
class EmailParser(object):
    def __init__(self, uid, password, dbname, host, port, model=False,
                 email_default=False):
...
def configure_parser():
    ...
    parser.add_option("--custom-values", dest="custom_values",
                      help="Dictionary of extra values to pass when creating"
                      " records",
                      default=None)
    parser.add_option("-s", dest="save_original",
                      action="store_true",
                      help="Keep a full copy of the email source attached to"
                      " each message",
                      default=False)
...
def main():
    ...
    try:
        ...
        subject = '[ODOO]:ERROR: Mailgateway - %s' % \
